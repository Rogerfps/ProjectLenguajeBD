...
BEGIN
    -- Ejecuta el procedimiento para actualizar el precio de un plato espec�fico
...
--------------------------------------------------------
--15. Procedimiento para obtener la lista de categor�as disponibles
...
BEGIN
    FOR categorias IN (SELECT descripcion FROM usuarioDondePapa.categoria WHERE disponible = 1) LOOP
        DBMS_OUTPUT.PUT_LINE('Categor�a: ' || categorias.descripcion);
    END LOOP;
END;

BEGIN
    -- Ejecuta el procedimiento para obtener las categor�as disponibles
    OBTENER_CATEGORIAS_DISPONIBLES;
END;
/
--------------------------------------------------------
--16.  Procedimiento para contar el n�mero de platos por categor�a
...
BEGIN
    FOR categoria IN (SELECT c.descripcion, COUNT(p.id_plato) AS numero_platos 
        ...
        DBMS_OUTPUT.PUT_LINE('Categor�a: ' || categoria.descripcion || ', N�mero de Platos: ' || categoria.numero_platos);
    END LOOP;
END;

BEGIN
    -- Ejecuta el procedimiento para contar los platos por categor�a
...
BEGIN
    ...
    INSERTAR_USUARIO(
        ...
        'password123',          -- Contrase�a del usuario
        'John',                 -- Nombre del usuario
        'Doe',                  -- Apellidos del usuario
        'john.doe@email.com',   -- Correo electr�nico del usuario
        '555-1234',             -- Tel�fono del usuario
...
BEGIN
    ...
    INSERTAR_USUARIO(
        ...
        'juan.perez@email.com',     -- Correo electr�nico del usuario
        '555-1234',                 -- Tel�fono del usuario
        '/images/JPeverest.jpeg',   -- Contrase�a del usuario
        1
    );
END;
/
...
BEGIN
    -- Prueba del procedimiento eliminando una factura espec�fica
    ELIMINAR_FACTURA(1); 
END;

--------------------------------------------------------
--19. Obtener Platos M�s Vendidos
...
-------------------------------------------------------
--20. Obtener usuario m�s activos
...
-------------------------------------------------------
--21. Generar reportes de ventas por categor�a
...
BEGIN
    ...
    LOOP
        DBMS_OUTPUT.PUT_LINE('Categor�a: ' || grupo_categorias.descripcion || ', Total ventas: ' || grupo_categorias.total_ventas);
    END LOOP;
END;

-------------------------------------------------------
--22. Restablecer contrase�a de usuario
...
-------------------------------------------------------
--25. Actualizar salarios en base a la cantidad de a�os laborados
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Salarios actualizados en funci�n de los a�os de servicio.');
END;
...
--------------------------------------------------------
//  7. Esta vista muestra los platos m�s vendidos, ordenados por la cantidad vendida.
...
ORDER BY 
    total_vendido DESC;
--------------------------------------------------------
//8. Esta vista muestra a los clientes m�s frecuentes, ordenados por el n�mero de facturas emitidas a su nombre.
...
BEGIN
  IF :new.ESTADO = 1 THEN
    ...
    VALUES (
      :new.ID_USUARIO,
      'VENTA', -- Puedes ajustar el tipo seg�n tus necesidades
      :new.TOTAL,
      SYSTIMESTAMP,
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Inserci�n 1 fallida: ' || SQLERRM);
END;
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Inserci�n 2 fallida: ' || SQLERRM);
END;
...
-- 3. Funci�n para verificar si una mesa est� apartada o reservada, esto 
-- introduciendo el n�mero de la mesa y la fecha del d�a de la reserva.
-- **ABIERTO A MEJORAS **
...
-- 4. Funci�n para encontrar el correo afiliado a un usuario espec�fico por medio de su ID.
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RETURN 'No se encontr� un correo afiliado';
END;
...
-- 5. Funci�n para obtener el nombre de un plato dependiendo del ID insertado
...
-- 7. Funci�n que muestra por medio de DBMS.OUTPUT, una lista de las transacciones
...
-- innecesario, esto puede ser de ayuda cuando se requiera hacer un an�lisis 
-- despu�s de la modificaci�n de precios en productos, ingreso/departura de otros, etc.
...
-- 11. Funcion para mostrar los primeros x (maximo) platos en ser los m�s popualres
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RETURN 'No tenemos esa reservaci�n registrada';
END;
...
-- 15. Esta funci�n va de la mano con la anterior, ya que una vez obtenido el
-- numero de telefono, se puede buscar el nombre y numero de mesa del usuario
-- que reserv� en caso de emergencia.

...
CREATE OR REPLACE PACKAGE usuarioDondePapa.PAQUETE_GESTION_FACTURAS IS
...
END PAQUETE_GESTION_FACTURAS;
/

---------------------------------------------------------
---------------------------------------------------------
-- 6. VENTAS
CREATE OR REPLACE PACKAGE usuarioDondePapa.PAQUETE_GESTION_VENTAS IS
  ...
  PROCEDURE calcular_total_usuario(
    p_id_usuario IN NUMBER,
    p_resultado OUT NUMBER
  );

  -- Generar informe de ventas
  PROCEDURE generar_informe_ventas(
    p_fecha_inicio IN DATE,
    p_fecha_fin IN DATE,
    p_resultado OUT SYS_REFCURSOR
  );

END PAQUETE_GESTION_VENTAS;
/
---------------------------------------------------------
---------------------------------------------------------
-- 7. ROLES
CREATE OR REPLACE PACKAGE usuarioDondePapa.PAQUETE_GESTION_ROLES IS
  ...
  PROCEDURE consultar_permisos_rol(
    p_id_rol IN NUMBER,
    p_resultado OUT SYS_REFCURSOR
  );

END PAQUETE_GESTION_ROLES;
/

---------------------------------------------------------
---------------------------------------------------------
-- 8. TRANSACCIONES
CREATE OR REPLACE PACKAGE usuarioDondePapa.PAQUETE_GESTION_TRANSACCIONES IS

  -- Registrar una nueva transacción
  PROCEDURE registrar_transaccion(
    p_id_usuario IN NUMBER,
    p_tipo IN VARCHAR2,
    p_monto IN NUMBER,
    p_fecha IN DATE,
  ...
  PROCEDURE consultar_transacciones_usuario(
    p_id_usuario IN NUMBER,
    p_resultado OUT SYS_REFCURSOR
  );

  -- Consultar transacciones por fecha
  PROCEDURE consultar_transacciones_fecha(
    p_fecha_inicio IN DATE,
    p_fecha_fin IN DATE,
    p_resultado OUT SYS_REFCURSOR
  );

  -- Calcular total de transacciones por tipo
  PROCEDURE calcular_total_transacciones_tipo(
    p_tipo IN VARCHAR2,
    p_resultado OUT NUMBER
  );

END PAQUETE_GESTION_TRANSACCIONES;
/
